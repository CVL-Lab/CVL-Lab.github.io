...
const LEGACY_PHOTO_METADATA = {
    ...
    20241127: {
        title: "Winter 2024 Lab Gathering",
    ...
    "20240826_2": {
        title: "August 2024 Graduation",
        caption: "August 2024 Graduation",
        date: "2024-08-26",
    },
    "20240826_1": {
        title: "Summer 2024 Mentoring Program",
        caption: "Summer 2024 Mentoring Program",
        date: "2024-08-26",
    },
    20240717: {
        title: "Summer 2024 Team Event",
        caption: "Summer 2024 Team Event",
        date: "2024-07-17",
    },
    20240222: {
        title: "February 2024 Graduation",
    ...
    "20231227_1": {
        title: "December Lab Seminar",
        caption: "December Lab Seminar",
        date: "2023-12-27",
    },
    "20230822_2": {
        title: "August 2023 Graduation",
...
const bootstrapNews = async () => {
    ...
    const contentFiles = existing.filter(
    ...
    const legacyNews = (await readJsonFile(LEGACY_NEWS_FILE, {
        items: [],
    })) ?? { items: [] };
    ...
    for (const item of items) {
        ...
        await writeMarkdown(
            ...
            `${id}.md`,
            {
                ...
                internal_slug:
                    normalizeText(item.internal_slug) || normalizeSlug(id),
            },
            summary,
        );
    }
    ...
    for (const [category, filePath] of Object.entries(
        ...
        for (const [legacyKey, entry] of publishedEntries) {
            ...
            const id = normalizeSlug(
                `${category}-${legacyKey}-${title.slice(0, 42)}`,
            );
            ...
            await writeMarkdown(
                ...
                `${id}.md`,
                {
                    ...
                    pdf_url: "",
                    arxiv_url: "",
                    github_url: normalizeText(meta.source_code_link),
                    project_url: normalizeText(meta.paper_link),
                    featured: false,
                    summary,
                },
                summary,
...
const bootstrapPhotos = async () => {
    const rawPhotos = await fs
        .readdir(PHOTOS_RAW_DIR, { withFileTypes: true })
        .catch(() => []);
    ...
    const targetDir = path.resolve(
        PHOTOS_RAW_DIR,
        "events",
        "2024-01-01__legacy-archive",
    );
    await ensureDir(targetDir);

    const legacyEntries = await fs.readdir(LEGACY_PHOTO_DIR, {
        withFileTypes: true,
    });
