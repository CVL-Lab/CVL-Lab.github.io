    ...
    return Object.entries(section.entries ?? {})
        .map(([id, person], index) => ({
            ...
            profile_details:
                ...
                typeof person.profile_details === "object"
                    ? {
                          biography: normalizeText(
                              person.profile_details.biography,
                          ),
                          research_overview: normalizeText(
                              person.profile_details.research_overview,
                          ),
                          education: normalizeText(
                              person.profile_details.education,
                          ),
                          affiliations: normalizeText(
                              person.profile_details.affiliations,
                          ),
                          responsibilities: normalizeText(
                              person.profile_details.responsibilities,
                          ),
                          history: normalizeText(
                              person.profile_details.history,
                          ),
                          achievements: normalizeText(
                              person.profile_details.achievements,
                          ),
                      }
                    : null,
...
export const countPeopleMembers = (sectionKeys = PEOPLE_SECTION_ORDER) =>
    sectionKeys.reduce(
        (sum, sectionKey) => sum + getPeopleEntriesBySection(sectionKey).length,
        0,
    );
